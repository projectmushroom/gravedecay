...
assert.equal(model.icon({platform:'container'}), 'os-logos/docker.svg');
assert.equal(model.icon({platform:'macos'}), 'os-logos/apple.svg');
assert.equal(model.icon({platform:'linux'}), 'os-logos/tux.svg');
assert.equal(model.icon({platform:'linux',os_icon:'archlinux'}), 'os-logos/archlinux.svg');
assert.equal(model.icon({platform:'linux',os_icon:'../../secret'}), 'os-logos/tux.svg');
const arch = {...mac,summary:{...summary,node:{platform:'linux',os_icon:'archlinux',os_name:'Omarchy'}}};
const restoredArch = model.restore(JSON.stringify({nodes:[arch]}))[0];
assert.equal(restoredArch.summary.node.os_icon, 'archlinux');
assert.equal(restoredArch.summary.node.os_name, 'Omarchy');
