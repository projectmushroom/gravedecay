...
Panel {
  ...
  function probeNext() {
    ...
    probing = pending.shift(); curlOutput = ""; curl.command = ["curl", "--silent", "--show-error", "--fail", "--connect-timeout", "2", "--max-time", "3", "--max-filesize", "65536", "https://" + probing.dns + "/grave/api/v1/summary"]; curl.running = true
  }
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      ColumnLayout { id: content; anchors.fill: parent; spacing: Style.space(8)
        ...
        Dropdown { visible: root.nodes.length > 1; width: parent.width; showLabel: false; options: root.nodes.map(function(n) { return { value: n.id, label: n.name } }); value: root.selectedId; onChanged: function(value) { root.selectedId = value } }
