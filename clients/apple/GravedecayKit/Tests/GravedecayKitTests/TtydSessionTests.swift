...
private final class Harness {
    ...
    func session(flow: TtydSession.FlowLimits = TtydSession.FlowLimits()) -> TtydSession {
        let session = TtydSession(sendFrame: { [self] in sentFrames.append($0) }, flow: flow)
        session.onOutput = { [self] output, done in outputs.append(output); pendingDone.append(done) }
        return session
    }
...
final class TtydSessionTests: XCTestCase {
    ...
    func testFlowControlPausesAndResumes() {
        ...
        session = harness.session(flow: flow)
    ...
    func testInputAndResizePassThrough() {
        session.send(bytes: [UInt8(ascii: "x")])
