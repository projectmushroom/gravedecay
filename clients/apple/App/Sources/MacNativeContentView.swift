...
struct MacNativeContentView: View {
    ...
    @StateObject private var model = MacDashboardModel()
...
private struct MacSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Linear") { SecureField("API key", text: $linearKey); Button("Save Linear key") { model.saveLinearKey(linearKey); linearKey = "" }; Text("Stored in this Mac’s Keychain. Assigned issues are read-only.").font(.footnote).foregroundStyle(.secondary) }
...
struct MacRepository: Identifiable { let path, name, branch, detail: String; let dirty: Bool; let github: String?; var id: String { path } }
struct MacSnapshot { let model, os, cpu, memory, disk, uptime, battery: String }
...
final class MacDashboardModel: ObservableObject {
    ...
    func saveLinearKey(_ key: String) { guard !key.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return }; Keychain.set(key, account: "linear-api-key"); refresh() }
    func refreshAsync() async {
        ...
        snapshot = result.snapshot; repositories = result.repositories; tailnetStatus = result.tailnetStatus; tailnetName = result.tailnetName; githubStatus = result.githubStatus; linearStatus = result.linearStatus; linearIssues = result.linearIssues
    }
}

private struct CollectionResult { let snapshot: MacSnapshot; let repositories: [MacRepository]; let tailnetStatus, tailnetName, githubStatus, linearStatus: String; let linearIssues: [String] }
private enum MacCollector {
    static func collect(root: String, linearKey: String?) -> CollectionResult {
        ...
        let uptime = command("sysctl", ["-n", "kern.boottime"]).flatMap { text in text.split(separator: " ").dropFirst().first }.map { _ in "Running" } ?? "—"
        let battery = command("pmset", ["-g", "batt"]).map { $0.contains("AC Power") ? "AC Power" : ($0.split(whereSeparator: { $0 == ";" }).first.map(String.init) ?? "Battery") } ?? "Not available"
        let snapshot = MacSnapshot(model: command("sysctl", ["-n", "hw.model"]) ?? "Mac", os: os, cpu: "\(ProcessInfo.processInfo.processorCount) cores", memory: memory, disk: used, uptime: uptime, battery: battery)
        ...
        return CollectionResult(snapshot: snapshot, repositories: repos, tailnetStatus: backend, tailnetName: name, githubStatus: github, linearStatus: linear.0, linearIssues: linear.1)
    }
    static func repositories(at root: String, ghAvailable: Bool) -> [MacRepository] {
        ...
        var found = [String]()
        while let url = enumerator.nextObject() as? URL, found.count < 24 {
        ...
        return found.sorted().enumerated().compactMap { index, path in
            ...
            return MacRepository(path: path, name: URL(fileURLWithPath: path).lastPathComponent, branch: branch, detail: detail, dirty: dirty, github: github)
        }
    }
    ...
    static func command(_ executable: String, _ arguments: [String], environment: [String: String] = [:]) -> String? {
        ...
        if done.wait(timeout: .now() + 2) == .timedOut { process.terminate(); _ = done.wait(timeout: .now() + 1) }
        pipe.fileHandleForReading.readabilityHandler = nil
        let remainder = pipe.fileHandleForReading.readDataToEndOfFile()
        lock.lock(); output.append(remainder); let data = output; let valid = !oversized && data.count <= 65_536; lock.unlock()
...
private enum Keychain {
    ...
    static func set(_ value: String, account: String) { let base: [CFString: Any] = [kSecClass: kSecClassGenericPassword, kSecAttrService: "com.projectmushroom.gravedecay", kSecAttrAccount: account]; SecItemDelete(base as CFDictionary); var query = base; query[kSecValueData] = Data(value.utf8); SecItemAdd(query as CFDictionary, nil) }
}
#endif
