...
def args_for_components():
 ...
 # restate it or it would silently converge the agents layer off.
 return (["--dashboard-only"] if n=="0" else ["--network-only"] if d=="0" else [])+(["--no-serve"] if s=="0" else [])+(["--agents"] if values.get("agents")=="1" else [])
