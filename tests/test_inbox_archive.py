...
from helpers import ROOT, load, run_notify
...
class InboxExecutionTests(unittest.TestCase):
    ...
    def run_notify(self, *args, configured=True):
        proc = run_notify(self.tmpdir.name, *args,
                          notify_env="NTFY_URL=https://ntfy.example/topic\n" if configured else None)
        return proc, pathlib.Path(self.tmpdir.name, "logs/notifications.jsonl")
...
class CollectorExecutionTests(unittest.TestCase):
    ...
    def test_inbox_newest_first_with_link_fallback(self):
        ...
        dash = load(ROOT / "dashboard/gravedecay.py", {"GRAVE_ROOT": str(self.root)})
